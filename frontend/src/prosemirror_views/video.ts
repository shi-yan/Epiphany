...
export default class VideoView implements NodeView {
    dom: HTMLElement
    node: PMNode
        ...
        this.input.addEventListener('keydown', (e: KeyboardEvent) => {
            if (e.code === 'ArrowUp') {
                ...
                if (targetPos !== undefined) {
                    const pos = this.outerView.state.doc.resolve(targetPos)
    ...
    deselectNode(): void {
        ...
        if (validatedUrl) {
            ...
            if (this.input.value === this.node.attrs.src) {
                ...
                this.display.innerHTML = '<iframe width="560" height="315" src="' + validatedUrl + '" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>'
            } else {
                setTimeout(() => {
                    const pos = this.getPos()
                    if (pos !== undefined) {
                        const tr = this.outerView.state.tr.setNodeAttribute(pos, 'src', validatedUrl)
                        this.outerView.dispatch(tr)
                    }
                }, 100)
            }
